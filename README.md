...
This step:
	...
	•	Extracts subjects, timestamps
	•	Cleans sender domains
	•	Builds a structured relational index
...
✔ Keyword size is based on frequency
...
• Download raw mbox emails from GMane (public archive)
• Store each email's header + body
• Output → content.sqlite


🧹 Step 2: Parse + Normalize
• Clean sender addresses
• Extract sender, subject, date, message-ID
...
• Count and rank frequencies
• Output → Python dictionary of keyword counts


🧪 Step 4: Generate JavaScript Dataset
• Convert frequencies to visual weights (20px–100px)
• Write final dataset to visualization/gword.js
• Output → JSON-like JS array for D3.js


🌈 Step 5: Interactive Visualization
• D3.js layout positions words
• Colors, rotations, and sizes applied dynamically
• Fully interactive UI
• Output → Word cloud visualization
```
